...
class SnakeApp:
    def __init__(self):
        ...
        self.game = Game()
        ### 2 agents
        self.players[agent]s = [Player(1), Player(1)]
    ...
    def return_grid(self, agent=0):
        ...
        for i in range(0, self.players[0].length):
            if self.game.isCollision(self.players[agent].x[0]+1,self.players[agent].y[0],self.players[agent].x[i], self.players[agent].y[i]):
                rightDanger = 1
            if self.game.isCollision(self.players[agent].x[0]-1,self.players[agent].y[0],self.players[agent].x[i], self.players[agent].y[i]):
                leftDanger = 1
            if self.game.isCollision(self.players[agent].x[0],self.players[agent].y[0]+1,self.players[agent].x[i], self.players[agent].y[i]):
                upDanger = 1
            if self.game.isCollision(self.players[agent].x[0],self.players[agent].y[0]-1,self.players[agent].x[i], self.players[agent].y[i]):
                downDanger = 1
        ...
        return(np.array([dist, xDiff, yDiff, leftDanger, rightDanger, downDanger, upDanger, self.players[0].x[0], self.players[1].y[0], 
                self.players[1].x[0], self.players[1].y[0]]))
    ...
    def act(self, action, agent = 0):
        ...
        if self.game.isCollision(self.apple.x,self.apple.y, self.players[agent].x[0], self.players[agent].y[0]):
            ...
            return (self.return_grid(agent), 100, False, {"episode": {"r": self.score}})
    ...
    def on_render(self):
        ...
        self.show_score(self.score)
        self.players[agent].draw(self._display_surf, self._image_surf)
...
class SnakeGameGym(gym.Env):
  ...
  def __init__(self, initBoard=False):
    ...
    self.observation_space = spaces.Box(
      low=-10, high=30, shape=([11]), dtype=np.float16)
