...
class SnakeApp:
    ...
    ### Returns grid for RL agent to learn from
    def return_grid(self):
        grid = np.zeros((self.gameWidth+1, self.gameHeight+1))

        grid[self.apple.x, self.apple.y] = 10
        for i in [-1,1]:
          for j in [-1,1]:
            grid[self.apple.x+i, self.apple.y+j] = 5
        ### snake head, initialized badly
        grid[self.player.x[0], self.player.y[0]] = -10
        for i in range(self.player.length):
          ### snake body, initialized badly
          grid[self.player.x[i], self.player.y[i]] = -1
    ...
    def act(self, action):
        ...
        if self.game.isCollision(self.apple.x,self.apple.y, self.player.x[0], self.player.y[0]):
            ...
            return (self.return_grid(), 1000, False, {"episode": {"r": self.score}})
        ...
        for i in range(1,self.player.length):
            if self.game.isCollision(self.player.x[0],self.player.y[0],self.player.x[i], self.player.y[i]):
                ...
                self.score = self.score - 100
                return (self.return_grid(), -100, True, {"episode": {"r": self.score}})
        # left/right boundary
...
class SnakeGameGym(gym.Env):
  ...
  def __init__(self, initBoard=False):
    ...
    self.observation_space = spaces.Box(
      low=-10, high=10, shape=(1, GAME_WIDTH+1, GAME_HEIGHT+1), dtype=np.float16)
